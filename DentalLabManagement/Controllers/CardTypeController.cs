...
namespace DentalLabManagement.API.Controllers
{
    ...
    public class CardTypeController : BaseController<CardTypeController>
    {
        ...
        [ProducesResponseType(typeof(CardTypeResponse), StatusCodes.Status200OK)]
        public async Task<IActionResult> GetCardTypes(int? categoryId, CardTypeStatus? status, int page, int size)
        {
            var response = await _cardTypeService.GetCardTypes(categoryId, status, page, size);
            return Ok(response);
        }
